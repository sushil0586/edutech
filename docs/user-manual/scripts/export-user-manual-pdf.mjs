...
function resolveImagePath(manualPath, src) {
  const normalized = src.replace(/^\.\//, "");
  return path.resolve(path.dirname(manualPath), normalized);
}

async function renderPdf({ inputPath, outputPath, title, eyebrow, summary }) {
  const markdown = await fs.readFile(inputPath, "utf8");
  const blocks = parseMarkdown(markdown);
  const body = [];
  body.push(
    `<section class="cover"><div class="eyebrow">${escapeHtml(eyebrow)}</div><h1>${escapeHtml(title)}</h1><p>${escapeHtml(summary)}</p></section>`,
  );
  ...
  for (const block of blocks) {
    if (block.type === "h1") {
      ...
      const absolute = resolveImagePath(inputPath, block.src);
      const buffer = await fs.readFile(absolute);
      const ext = path.extname(absolute).toLowerCase();
      const mime = ext === ".png" ? "image/png" : ext === ".jpg" || ext === ".jpeg" ? "image/jpeg" : "application/octet-stream";
      body.push(
        `<figure><img alt="${escapeHtml(block.alt)}" src="data:${mime};base64,${buffer.toString("base64")}" /><figcaption>${escapeHtml(block.alt)}</figcaption></figure>`,
      );
    }
  }
...
async function main() {
  ...
  const eyebrow = args.eyebrow ?? "Nexora Workflow Manual";
  const summary = args.summary ?? "";

  if (!input || !output || !title || !summary) {
    console.error(
      "Usage: node export-user-manual-pdf.mjs --input <markdown> --output <pdf> --title <title> --summary <summary> [--eyebrow <eyebrow>]",
    );
    process.exit(1);
  }
  ...
  await renderPdf({ inputPath, outputPath, title, eyebrow, summary });
}
