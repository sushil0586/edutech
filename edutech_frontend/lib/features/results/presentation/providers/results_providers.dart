...
final studentResultRecordsProvider =
    FutureProvider.autoDispose<List<ExamResultModel>>((ref) {
...
final studentPerformanceProvider = FutureProvider.autoDispose
    .family<List<ExamResultModel>, String>((ref, studentId) {
      return ref
          .watch(resultsRepositoryProvider)
          .fetchStudentPerformance(studentId);
    });

final teacherExamSummariesProvider =
    FutureProvider.autoDispose<List<ExamSummaryModel>>((ref) {
      return ref.watch(resultsRepositoryProvider).fetchTeacherResultSummaries();
    });

final examSummaryRecordsProvider =
    FutureProvider.autoDispose<List<ExamSummaryModel>>((ref) {
      return ref.watch(resultsRepositoryProvider).fetchExamSummaries();
    });

final examLeaderboardProvider = FutureProvider.autoDispose
    .family<List<LeaderboardRowModel>, String>((ref, examId) {
      return ref.watch(resultsRepositoryProvider).fetchExamLeaderboard(examId);
    });

final liveExamMonitorProvider = FutureProvider.autoDispose
    .family<LiveExamMonitorModel, String>((ref, examId) {
      return ref.watch(resultsRepositoryProvider).fetchLiveExamMonitor(examId);
    });

final teacherExamAttemptsProvider = FutureProvider.autoDispose
    .family<List<TeacherExamAttemptModel>, String>((ref, examId) {
      return ref.watch(resultsRepositoryProvider).fetchExamAttempts(examId);
    });

final teacherQuestionAnalysisProvider = FutureProvider.autoDispose
    .family<List<TeacherQuestionAnalysisModel>, String>((ref, examId) {
      return ref.watch(resultsRepositoryProvider).fetchQuestionAnalysis(examId);
    });

final topicPerformanceProvider = FutureProvider.autoDispose
    .family<List<TopicPerformanceModel>, TopicPerformanceQuery>((ref, query) {
      return ref
          .watch(resultsRepositoryProvider)
