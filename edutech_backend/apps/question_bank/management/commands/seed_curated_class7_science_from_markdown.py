...
class Command(BaseCommand):
    help = (
        "Lint, compile, and seed all curated Class 7 Science topic packs from Markdown "
        "using the Markdown-first authoring workflow."
    )

    def add_arguments(self, parser):
        parser.add_argument("institute_code", help="Institute code to seed curated Science questions into.")
        parser.add_argument(
            "--preset",
            default="class_7_cbse_core",
            help="Academic preset used to discover Class 7 Science leaf topics.",
        )
        parser.add_argument(
            "--topic-codes",
            nargs="*",
            default=[],
            help="Optional specific Science topic codes to process. Defaults to all Class 7 Science leaf topics.",
        )
        parser.add_argument(
            "--expected-count",
            type=int,
        ...
        parser.add_argument(
            "--replace-existing",
            action="store_true",
            help="Replace the existing curated seed batch for the selected Science topics.",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Step 1/3: linting {len(file_args)} Class 7 Science Markdown pack(s).")
        call_command(
        ...
        self.stdout.write(f"Step 2/3: compiling {len(file_args)} Class 7 Science Markdown pack(s).")
        call_command(
        ...
        self.stdout.write(f"Step 3/3: seeding {len(topic_codes)} Class 7 Science topic pack(s).")
        call_command(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Completed Markdown-first Class 7 Science seed flow for {len(topic_codes)} topic(s)."
            )
        )
    ...
    def _resolve_topic_codes(self, *, preset, selected_topic_codes):
        ...
        discovered_topics = list(
            iter_leaf_topics(
                preset,
                subject_aliases=["science"],
                topic_codes=normalized_codes,
            )
        )
        if not discovered_topics:
            raise CommandError("No matching Class 7 Science leaf topics were found.")
