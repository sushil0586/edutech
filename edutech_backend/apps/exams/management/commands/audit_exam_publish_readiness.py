...
from apps.exams.services import build_exam_publish_readiness


class Command(BaseCommand):
    help = "Audit publish readiness across active exams and optionally fail when blockers exist."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--fail-on-blockers",
            action="store_true",
            help="Exit with a non-zero status code when any exam has blockers.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["fail_on_blockers"] and blocker_exam_count:
            raise CommandError(
                f"Exam publish readiness audit found blockers in {blocker_exam_count} exam(s)."
            )
