...
test.describe("Teacher NEET results contract", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher sees both seeded NEET exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "teacher");
    await expectTeacherWorkspace(page);

    const fullMock = await fetchTeacherExamByCode(page, neetFullMockCode);
    const publishedMock = await fetchTeacherExamByCode(page, neetPublishedMockCode);
