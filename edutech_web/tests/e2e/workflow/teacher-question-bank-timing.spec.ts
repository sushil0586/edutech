...
test.describe("Teacher question bank timing", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher question bank route timing probe stays measurable", async ({ page }, testInfo: TestInfo) => {
    ...
    await measureTiming({
      ...
      metrics,
      action: async () => {
        await page.goto("/teacher/question-bank/import", { waitUntil: "domcontentloaded" });
      },
      assertVisible: async () => {
    ...
    await measureTiming({
      ...
      metrics,
      action: async () => {
        await page.goto("/teacher/question-bank/new", { waitUntil: "domcontentloaded" });
      },
      assertVisible: async () => {
    ...
    await testInfo.attach("teacher-question-bank-timing", {
      body: Buffer.from(JSON.stringify(payload, null, 2)),
      contentType: "application/json",
    });
    console.log("teacher-question-bank-timing", JSON.stringify(payload));
  });
});
