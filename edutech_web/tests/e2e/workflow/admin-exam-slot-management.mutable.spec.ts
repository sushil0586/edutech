...
test.describe("Admin exam slot management", () => {
  ...
  test.skip(
    ...
    mutableLaneMessage(
      "PLAYWRIGHT_ENABLE_MUTABLE_ADMIN_EXAM_DETAIL_ACTIONS",
      "disposable admin slot-management coverage",
    ),
  );

  test("@workflow @mutable admin can manage slot-based access windows and student overrides from exam detail", async ({
    page,
  }) => {
    ...
    try {
      const created = await createAdminFamilyExamDirectly(page, deterministicWizardScenario, uniqueSeed, {
        titlePrefix: "PW Admin Slots",
        codePrefix: "PWASM",
      });
      ...
      let detail = await fetchAdminExamDetail(page, examId);
      ...
      detail = await fetchAdminExamDetail(page, examId);
