...
async function createDisposableTrueFalseQuestion(page: Page, programName: string, questionText: string) {
  ...
  await page.locator('textarea[name="explanation"]').fill("Disposable objective prompt for mixed runtime coverage.");
...
async function createDisposableDescriptiveQuestion(page: Page, programName: string, questionText: string) {
  ...
  await page
    ...
    .fill("Disposable descriptive prompt for mixed runtime coverage.");
  await page.locator('textarea[name="review_guidance"]').fill("Reward a clear answer with one concrete example.");
...
test.describe("Student mixed question-type runtime actions", () => {
  test.skip(
  ...
  test.skip(
    ...
    mutableLaneMessage(
      "PLAYWRIGHT_ENABLE_MUTABLE_STUDENT_ATTEMPT_ACTIONS",
      "mixed question-type student runtime coverage",
    ),
  );

  test("@workflow @mutable student can move between objective and descriptive questions in one attempt", async ({
    page,
  }) => {
    ...
    const objectiveQuestionText = `PW mixed runtime objective question ${uniqueSeed}`;
    const descriptiveQuestionText = `PW mixed runtime descriptive question ${uniqueSeed}`;
    const descriptiveAnswerText = `mixed descriptive answer ${uniqueSeed}`;
    const examTitle = `PW Student Mixed Runtime ${uniqueSeed}`;
    const examCode = `PW-SMR-${uniqueSeed}`;
    ...
    try {
      ...
      const objectiveQuestion = await createDisposableTrueFalseQuestion(page, studentProgramName!, objectiveQuestionText);
      questionSubjectLabel = objectiveQuestion.subjectLabel;
      await createDisposableDescriptiveQuestion(page, studentProgramName!, descriptiveQuestionText);
      ...
      const objectiveOption = await questionSelect.locator("option").evaluateAll((options, expectedText) =>
        options
          .map((option) => ({
      ...
      expect(objectiveOption).not.toBeNull();
      await questionSelect.selectOption(objectiveOption!.value);
      ...
      const descriptiveOption = await questionSelect.locator("option").evaluateAll((options, expectedText) =>
        options
          .map((option) => ({
            label: (option as HTMLOptionElement).label,
            value: (option as HTMLOptionElement).value,
        ...
        descriptiveQuestionText,
      );
      expect(descriptiveOption).not.toBeNull();
      await questionSelect.selectOption(descriptiveOption!.value);
      ...
      await answerCurrentAttemptQuestion(page, uniqueSeed, "mixed descriptive answer");
      ...
      const secondQuestionChip = page
        .locator(".attemptQuestionNavChip")
        .filter({ has: page.locator("strong", { hasText: /^2$/ }) })
        .first();
      await expect(secondQuestionChip).toBeVisible();
