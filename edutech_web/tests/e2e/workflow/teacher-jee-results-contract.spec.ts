...
test.describe("Teacher JEE results contract", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher sees both seeded JEE exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "teacher");
    await expectTeacherWorkspace(page);

    const fullMock = await fetchTeacherExamByCode(page, jeeFullMockCode);
    const publishedMock = await fetchTeacherExamByCode(page, jeePublishedMockCode);

    await page.goto(`/teacher/exams/${fullMock.id}`);
    ...
    await page.goto(`/teacher/results?exam=${encodeURIComponent(publishedMock.id)}`);
