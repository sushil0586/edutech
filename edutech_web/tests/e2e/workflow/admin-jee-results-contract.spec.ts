...
const jeeFullMockCode = "DMO-JEE-FULL-01";
const jeePublishedMockCode = "DMO-JEE-RESULT-01";
...
test.describe("Admin JEE results contract", () => {
  test.skip(testRequiresRole("admin"), "Admin Playwright credentials are not configured.");

  test("@workflow admin sees both seeded JEE exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "admin");
    await expectAdminWorkspace(page);

    const fullMock = await fetchAdminExamByCode(page, jeeFullMockCode);
    const publishedMock = await fetchAdminExamByCode(page, jeePublishedMockCode);

    await page.goto(`/admin/exams/${fullMock.id}`);
    ...
    await page.goto(`/admin/exams/${publishedMock.id}`);
    await expect(page.getByRole("heading", { name: new RegExp(publishedMock.title, "i") }).first()).toBeVisible();
    ...
    await expect(page.getByText(publishedMock.subject_summary!.display_label!).first()).toBeVisible();
