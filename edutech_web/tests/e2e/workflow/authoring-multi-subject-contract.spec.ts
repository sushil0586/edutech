...
async function fetchTeacherExamList(page: Page) {
  const accessToken = await backendAccessToken(page);
  const response = await page.request.get(
    `${backendBaseUrl}/api/v1/teacher/exams/?search=${encodeURIComponent(multiSubjectExamCode)}&page_size=20`,
    {
      headers: {
...
async function verifyRoleContract(args: {
  page: Page;
  role: "teacher" | "institute";
  detailPathPrefix: "/teacher/exams" | "/institute/exams";
}) {
  const { page, role, detailPathPrefix } = args;
  ...
  const listPayload = await fetchTeacherExamList(page);
  ...
  const detailPayload = await fetchTeacherExamDetail(page, exam!.id);
  ...
  await page.goto(`${detailPathPrefix}/${exam!.id}`);
...
test.describe("Authoring multi-subject contract", () => {
  ...
  test("@workflow teacher sees the seeded mixed-subject exam as a real multi-subject authoring record", async ({
    page,
  }) => {
    await verifyRoleContract({
      page,
      role: "teacher",
      detailPathPrefix: "/teacher/exams",
    });
  });

  test("@workflow institute sees the seeded mixed-subject exam as a real multi-subject authoring record", async ({
    page,
  }) => {
    await verifyRoleContract({
      page,
      role: "institute",
      detailPathPrefix: "/institute/exams",
    });
  });
});
