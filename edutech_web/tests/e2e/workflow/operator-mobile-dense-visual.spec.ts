...
test.describe("Operator mobile dense visual contracts", () => {
  ...
  test("@workflow @visual teacher mobile exam detail keeps hero, KPI strip, and delivery actions aligned", async ({
    ...
    const hero = await firstVisible([
      page.locator("main > div", {
        has: page.getByRole("heading", { level: 1 }).first(),
      }).first(),
    ...
    const deliveryActions = await firstVisible([page.locator("#exam-actions").first()]);

    await expectVisualSnapshot(hero, "teacher-mobile-exam-detail-hero.png", 420);
    await expectVisualSnapshot(kpiStrip, "teacher-mobile-exam-detail-kpi-strip.png", 420);
  ...
  test("@workflow @visual institute mobile exam detail keeps hero, KPI strip, and delivery actions aligned", async ({
    ...
    await expectVisualSnapshot(deliveryActions, "institute-mobile-exam-detail-delivery-actions.png", 560);
    await expectVisualSnapshot(primaryActionSurface, "institute-mobile-exam-detail-action-grid.png", 520);
  });
});
