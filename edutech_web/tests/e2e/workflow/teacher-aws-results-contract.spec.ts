...
test.describe("Teacher AWS results contract", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher sees both seeded AWS exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "teacher");
    await expectTeacherWorkspace(page);

    const liveExam = await fetchTeacherExamByCode(page, awsLiveCode);
    const publishedExam = await fetchTeacherExamByCode(page, awsPublishedCode);
