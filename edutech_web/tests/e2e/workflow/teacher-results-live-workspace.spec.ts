...
async function expectTeacherLiveMonitorWorkspace(page: Page) {
...
test.describe("Teacher live monitor workspace", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher can inspect live monitor controls and drill into an attempt when available", async ({
    page,
  }) => {
    ...
    await expectTeacherLiveMonitorWorkspace(page);
