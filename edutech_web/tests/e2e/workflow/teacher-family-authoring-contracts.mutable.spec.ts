...
async function buildQuestionPayload(page: Page, program: ProgramRegistryRecord, questionType: string, uniqueSeed: number) {
  const authProfile = await fetchAuthProfile(page);
  expect(authProfile.institute).not.toBeNull();
  expect(authProfile.teacher_profile).not.toBeNull();
  ...
  return {
    ...
    subject: subject!.id,
    topic: topic?.id ?? null,
    created_by_teacher: authProfile.teacher_profile,
    question_type: questionType,
    difficulty_level: "intermediate",
    content_format: "plain_text",
    question_text: `PW family contract ${questionType} ${uniqueSeed}`,
    explanation: "Family contract automation question.",
    accepted_answers:
...
test.describe("Teacher family authoring contracts", () => {
  test.skip(
    testRequiresRole("teacher"),
    "Teacher Playwright credentials are not configured.",
  );

  test("@workflow teacher question editor filters family question types and scoring hints", async ({
    page,
  }) => {
  ...
  test("@workflow @mutable teacher question API enforces competitive, certification, and language family contracts", async ({
    ...
    try {
      ...
      const invalidCompetitiveBody = (await invalidCompetitiveResponse.json()) as { question_type?: string | string[] };
      ...
      const invalidCertificationBody = (await invalidCertificationResponse.json()) as { question_type?: string | string[] };
      ...
      const invalidLanguageBody = (await invalidLanguageResponse.json()) as { question_type?: string | string[] };
