...
test.describe("Student mutable analytics drill-down continuity", () => {
  ...
  test("@workflow @mutable student analytics drill-downs preserve scoped result context across compare timeline actions and subject views", async ({
    ...
    const questionText = `PW student analytics drilldown question ${uniqueSeed}`;
    const examTitle = `PW Student Drilldown ${uniqueSeed}`;
    const examCode = `PW-SAD-${uniqueSeed}`;
    const now = new Date();
    ...
    try {
      ...
      await page.locator('textarea[name="explanation"]').fill(
        "Disposable explanation for student analytics drill-down continuity coverage.",
      );
      ...
      await answerCurrentAttemptQuestion(page, uniqueSeed, "Playwright student analytics drilldown answer");
      ...
      const analyticsResultLink = page
        .locator(".analyticsPanelResults a.dashboardRailRow")
        .filter({ has: page.getByText(new RegExp(escapeRegExp(examTitle), "i")) })
        .first();
      await expect(analyticsResultLink).toBeVisible();

      const analyticsResultHref = await analyticsResultLink.getAttribute("href");
      ...
      await analyticsResultLink.click();
      ...
      await expect(openSubjectDeepDiveLink).toBeVisible();
