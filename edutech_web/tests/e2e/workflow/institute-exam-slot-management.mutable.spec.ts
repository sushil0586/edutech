...
import { createInstituteFamilyExamDirectly, familyRuntimeScenarios } from "../helpers/family-runtime";
...
async function deleteInstituteExamDirectly(page: Page, examId: string) {
...
type InstituteExamDetailResponse = {
  id: string;
  title: string;
  code: string;
...
async function fetchInstituteExamDetail(page: Page, examId: string) {
  const accessToken = await backendAccessToken(page);
  const response = await page.request.get(`${instituteApiBaseUrl}/api/v1/exams/${examId}/`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json",
    },
  ...
  expect(response.ok(), await response.text()).toBe(true);
  return (await response.json()) as InstituteExamDetailResponse;
}
...
async function fetchScopedStudents(page: Page, args: {
  ...
  const response = await page.request.get(`${instituteApiBaseUrl}/api/v1/students/?${searchParams.toString()}`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json",
    },
...
test.describe("Institute exam slot management", () => {
  ...
  test.skip(
    ...
    mutableLaneMessage(
      "PLAYWRIGHT_ENABLE_MUTABLE_EXAM_ACTIONS",
      "disposable institute slot-management coverage",
    ),
  );

  test("@workflow @mutable institute can manage slot-based access windows and student overrides from exam detail", async ({
    page,
  }) => {
    ...
    try {
      const created = await createInstituteFamilyExamDirectly(page, deterministicWizardScenario, uniqueSeed, {
        titlePrefix: "PW Institute Slots",
        codePrefix: "PWISM",
      });
      examId = created.examId;

      const initialDetail = await fetchInstituteExamDetail(page, examId);
      ...
      let detail = await fetchInstituteExamDetail(page, examId);
      ...
      detail = await fetchInstituteExamDetail(page, examId);
