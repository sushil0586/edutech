...
test.describe("Teacher advanced builder browser button coverage", () => {
  test("@workflow browser coverage exercises teacher advanced-builder buttons end-to-end", async ({
    ...
    await sectionCards.last().getByRole("button", { name: /^remove$/i }).click();
    ...
    await topicRows.last().getByRole("button", { name: /^remove$/i }).click();
    ...
    await page.route("**/api/exams/preset-packs*", async (route) => {
      ...
      if (request.method() === "POST") {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            id: "pw-teacher-managed-pack-browser-coverage",
          }),
        });
        return;
      }
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          results: [
            {
              id: "pw-teacher-managed-pack-browser-coverage",
              label: "PW Teacher Managed Pack Browser Coverage",
              family: "Custom",
              note: "Teacher managed pack browser coverage fixture",
              chip: "Managed",
              scope_type: "institute",
              can_manage: true,
              resourceId: "pw-teacher-managed-pack-browser-coverage-resource",
            },
          ],
        }),
      });
  ...
  test("@workflow browser coverage keeps teacher create blocked when preview returns composition errors", async ({
    page,
  }) => {
    await openAdvancedBuilder(page, {
      credentials: teacherCredentials,
