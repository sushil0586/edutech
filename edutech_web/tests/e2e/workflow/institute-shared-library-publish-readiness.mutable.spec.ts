...
const mutableInstituteSharedLibraryPublishReadinessEnabled = isMutableLaneEnabled(
  "PLAYWRIGHT_ENABLE_MUTABLE_INSTITUTE_SHARED_LIBRARY_PUBLISH_READINESS",
);
...
async function createInstituteExamWithLinkedQuestion(page: Page, options: {
  examTitle: string;
  examCode: string;
  sectionName: string;
  searchProbe: string;
...
async function configureInstituteExamSchedule(page: Page, examId: string) {
...
async function publishInstituteExam(page: Page, examId: string) {
...
test.describe("Institute shared-library publish readiness", () => {
  test.skip(
  ...
  test.skip(
    !mutableInstituteSharedLibraryPublishReadinessEnabled,
    mutableLaneMessage(
      "PLAYWRIGHT_ENABLE_MUTABLE_INSTITUTE_SHARED_LIBRARY_PUBLISH_READINESS",
      "institute shared-library publish readiness coverage",
    ),
  );

  test("@workflow @mutable institute exam detail shows paused shared-library entitlement as a publish blocker", async ({
    page,
  }) => {
    ...
    const examTitle = `PW Shared Publish Readiness ${uniqueSeed}`;
    const examCode = `PW-SPR-${uniqueSeed}`;
    const sectionName = `Publish Readiness Section ${uniqueSeed}`;
    ...
    examId = await createInstituteExamWithLinkedQuestion(page, {
      examTitle,
      examCode,
      sectionName,
      searchProbe,
    });
    await configureInstituteExamSchedule(page, examId);
    ...
    try {
      for (const entitlement of targetEntitlements) {
        const pauseResponse = await page.request.patch(
          `${instituteApiBaseUrl}/api/v1/economy/admin/question-bank-entitlements/${entitlement.id}/`,
          {
            ...
            data: {
              status: "paused",
              notes: "Playwright publish readiness check paused this entitlement temporarily.",
            },
          },
        );
      ...
      const readinessPanel = instituteExamReadinessPanel(page);
      ...
      for (const entitlement of targetEntitlements) {
        const reactivateResponse = await page.request.patch(
          `${instituteApiBaseUrl}/api/v1/economy/admin/question-bank-entitlements/${entitlement.id}/`,
          {
            ...
            data: {
              status: "active",
              notes: "Playwright publish readiness check restored this entitlement.",
            },
          },
        );
  ...
  test("@workflow @mutable institute exam detail shows shared-library publish allowance warnings and blockers from package metadata", async ({
    page,
  }) => {
    ...
    const uniqueSeed = Date.now();
    const seedExamTitle = `PW Shared Publish Limit Seed ${uniqueSeed}`;
    const seedExamCode = `PW-SPLS-${uniqueSeed}`;
    const draftExamTitle = `PW Shared Publish Limit Draft ${uniqueSeed}`;
    const draftExamCode = `PW-SPLD-${uniqueSeed}`;
    const seedSectionName = `Publish Limit Seed Section ${uniqueSeed}`;
    const draftSectionName = `Publish Limit Draft Section ${uniqueSeed}`;
    ...
    if (!linkableCard) {
      test.skip(true, "No institute-visible shared-library card currently exposes Link to Local Bank.");
    }
    ...
    const packagesResponse = await page.request.get(
      `${instituteApiBaseUrl}/api/v1/economy/admin/question-bank-packages/`,
      {
        headers: {
          Authorization: `Bearer ${adminAccessToken}`,
        },
    ...
    try {
      await loginAsRole(page, "institute");
      await expectInstituteWorkspace(page);
      const institutePublishAccessToken = await getAccessToken(page);
      expect(institutePublishAccessToken).not.toBe("");
      ...
      const nearLimitResponse = await page.request.patch(
        `${instituteApiBaseUrl}/api/v1/economy/admin/question-bank-packages/${packageId}/`,
        {
          headers: {
            Authorization: `Bearer ${adminAccessToken}`,
            "Content-Type": "application/json",
      ...
      await configureInstituteExamSchedule(page, seedExamId);
      ...
      await configureInstituteExamSchedule(page, draftExamId);

      await page.goto(`/institute/exams/${draftExamId}`);
      const readinessPanel = instituteExamReadinessPanel(page);
      await expect(readinessPanel).toBeVisible();
      ...
      if (draftExamId) {
        await loginAsRole(page, "institute");
        await expectInstituteWorkspace(page);
        await deleteInstituteExam(page, draftExamId);
      }

      if (seedExamId) {
        await loginAsRole(page, "institute");
        await expectInstituteWorkspace(page);
        await deleteInstituteExam(page, seedExamId);
      }
