...
test.describe("Admin NEET results contract", () => {
  test.skip(testRequiresRole("admin"), "Admin Playwright credentials are not configured.");

  test("@workflow admin sees both seeded NEET exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "admin");
    await expectAdminWorkspace(page);

    const fullMock = await fetchAdminExamByCode(page, neetFullMockCode);
    const publishedMock = await fetchAdminExamByCode(page, neetPublishedMockCode);
