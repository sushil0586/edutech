...
test.describe("Teacher shared-library no-entitlement workspace", () => {
  test.skip(testRequiresRole("teacher"), "Teacher Playwright credentials are not configured.");

  test("@workflow teacher sees truthful blocked shared-library state when no matching package exists", async ({
    page,
  }) => {
