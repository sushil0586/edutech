...
test.describe("Institute JEE results contract", () => {
  test.skip(testRequiresRole("institute"), "Institute Playwright credentials are not configured.");

  test("@workflow institute sees both seeded JEE exams as real oversight records", async ({ page }) => {
    await loginAsRole(page, "institute");
    await expectInstituteWorkspace(page);

    const fullMock = await fetchInstituteExamByCode(page, jeeFullMockCode);
    const publishedMock = await fetchInstituteExamByCode(page, jeePublishedMockCode);
