...
async function gotoSearch(page: Page, path = "/teacher/search?q=exam") {
...
test.describe("Teacher search browser functionality coverage", () => {
  ...
  test("@workflow browser coverage keeps teacher search controls hydrated", async ({ page }) => {
  ...
  test("@workflow browser coverage can apply and reset teacher search filters truthfully", async ({
    page,
  }) => {
    await gotoSearch(page);

    await sourceSelect(page).selectOption("catalog");
  ...
  test("@workflow browser coverage keeps teacher search quick filters truthful", async ({ page }) => {
    await gotoSearch(page);
    ...
    await expect(page).toHaveURL(/\/teacher\/search\?q=exam$/);
    await expect(sourceSelect(page)).toHaveValue("all");
    await expect(groupSelect(page)).toHaveValue("none");
  });

  test("@workflow browser coverage proves teacher search no-results state is distinct from normal results", async ({
    page,
  }) => {
    await gotoSearch(page, "/teacher/search?q=playwright-definitely-no-match-zzzz&source=live&group=section");
  ...
  test("@workflow browser coverage keeps teacher search summary counts internally truthful", async ({
    page,
  }) => {
    await gotoSearch(page);
