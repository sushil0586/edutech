...
async function updateQuestionAction(formData: FormData) {
  ...
  if (!profile.institute || !questionId) {
    redirect("/institute/question-bank?error=Question%20context%20is%20missing.");
  }
...
async function addQuestionTagAction(formData: FormData) {
  ...
  if (!questionId || !tagId) {
    redirect(`/institute/question-bank/${questionId || ""}?error=${encodeURIComponent("Choose a tag before saving.")}`);
  }
...
async function removeQuestionAttachmentAction(formData: FormData) {
  ...
  if (!questionId || !attachmentId) {
    redirect(
      `/institute/question-bank/${questionId || ""}?error=${encodeURIComponent("Attachment context is missing.")}`,
    );
  }
...
async function removeQuestionTagAction(formData: FormData) {
  ...
  if (!questionId || !tagMapId) {
    redirect(`/institute/question-bank/${questionId || ""}?error=${encodeURIComponent("Tag context is missing.")}`);
  }
...
export default async function InstituteQuestionDetailPage({
  params,
  searchParams,
}: {
    ...
    return (
      <div className="studentPage">
        ...
        <StudentStatePanel
          ...
          description="The selected question was not available from the institute question bank, or the academic lookup endpoints did not complete successfully."
          bullets={[
  ...
  if (isReadOnlyLibraryQuestion(question)) {
    redirect(
      `/institute/question-bank/new?duplicate=${question.id}&error=${encodeURIComponent(
        "Public library questions are read-only here. Duplicate the question to create an editable institute copy.",
      )}`,
    );
  }
          ...
          <section className="builderSectionCard">
            <div className="builderSectionHeader">
              <div>
                ...
                <p>
                  Organize this question with institute-facing tags so it is easier to
                ...
                <div>
                  ...
                  <span>
                    {availableTags.length
                      ? `${availableTags.length} more active tags are available in the institute scope.`
