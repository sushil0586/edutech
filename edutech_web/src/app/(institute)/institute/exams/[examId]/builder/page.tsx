...
function buildBuilderReadinessSnapshot(args: {
  ...
  if (activeSectionsCount > 0) {
    ready.push(`${activeSectionsCount} section${activeSectionsCount === 1 ? "" : "s"} configured.`);
  } else {
    blockers.push("No active sections are configured yet.");
  }

  if (activeQuestionCount > 0) {
    ready.push(`${activeQuestionCount} linked question${activeQuestionCount === 1 ? "" : "s"} mapped into the exam.`);
  } else {
    blockers.push("No linked questions are mapped yet.");
  }
  ...
  if (examStatus === "draft") {
    pending.push("Exam lifecycle is still draft. Publish when setup and review readiness are complete.");
...
async function updateExamSettingsAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/institute/exams/${examId}/builder?error=${encodeURIComponent(message)}`);
  }

  redirect(`/institute/exams/${examId}/builder?message=${encodeURIComponent("Exam settings updated.")}`);
}
...
async function addSectionAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/institute/exams/${examId}/builder?tab=sections&error=${encodeURIComponent(message)}`);
  }

  redirect(`/institute/exams/${examId}/builder?tab=sections&message=${encodeURIComponent("Section added.")}`);
}
...
async function deleteSectionAction(formData: FormData) {
  ...
  redirect(`/institute/exams/${examId}/builder?tab=sections&message=${encodeURIComponent("Section removed.")}`);
}
...
async function addQuestionLinkAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/institute/exams/${examId}/builder?tab=questions&error=${encodeURIComponent(message)}`);
  }

  redirect(`/institute/exams/${examId}/builder?tab=questions&message=${encodeURIComponent("Question linked to exam.")}`);
}
...
async function bulkAddQuestionLinksAction(formData: FormData) {
  ...
  if (!questionIds.length) {
    redirect(`/institute/exams/${examId}/builder?tab=questions&error=${encodeURIComponent("Select at least one question to attach.")}`);
  }
  ...
  redirect(
    `/institute/exams/${examId}/builder?tab=questions&message=${encodeURIComponent(
...
async function deleteQuestionLinkAction(formData: FormData) {
  ...
  redirect(`/institute/exams/${examId}/builder?tab=questions&message=${encodeURIComponent("Linked question removed.")}`);
}
...
async function updateQuestionLinkAction(formData: FormData) {
  ...
  redirect(`/institute/exams/${examId}/builder?tab=questions&message=${encodeURIComponent("Linked question updated.")}`);
}
...
async function updateAssignmentsAction(formData: FormData) {
  ...
  redirect(`/institute/exams/${examId}/builder?tab=assignment&message=${encodeURIComponent("Student assignment updated.")}`);
}
...
async function updateStudentAccommodationAction(formData: FormData) {
  ...
  redirect(
    `/institute/exams/${examId}/builder?tab=assignment&message=${encodeURIComponent(
...
export default async function InstituteExamBuilderPage({
  params,
  searchParams,
}: {
  ...
  if (!builderData) {
    return (
      <div className="studentPage">
        <InstitutePageHeader
          title="Exam Builder"
          description="This route depends on live institute-scoped exam builder data from the backend."
        />
        <StudentStatePanel
          eyebrow="Load issue"
    ...
    },
    {
      ...
      content: (
        <BuilderQuestionMapping
          academicSetupHref="/institute/question-bank"
      ...
      content: (
        <article className="dashboardPanel builderPanel" id="student-assignment">
          ...
          <div className="builderHintPanel assignmentWorkflowGuide">
            ...
            <p>
              Start with learner targeting, then move into access slots, overrides, and support.
              Each section below focuses on one operational job so you do not have to scan the whole page every time.
            </p>
            <div className="assignmentWorkflowNav">
              <a className="button buttonGhost" href="#assignment-roster">Learners</a>
              <a className="button buttonGhost" href="#assignment-slots">Access Slots</a>
              <a className="button buttonGhost" href="#assignment-overrides">Overrides</a>
              <a className="button buttonGhost" href="#assignment-audit">Audit</a>
              <a className="button buttonGhost" href="#assignment-support">Accommodation</a>
            </div>
          </div>

          <section className="builderSectionCard" id="assignment-roster">
            <div className="builderPanelHeader">
              <div>
                <span className="builderFlowLabel">Step 1</span>
                <strong>Choose who receives this exam</strong>
                <p>Set the assignment mode, then confirm the learner list before you touch slot routing or overrides.</p>
              </div>
            </div>
          ...
          <section className="builderSectionCard" id="assignment-slots">
            <div className="builderPanelHeader">
              <div>
                <span className="builderFlowLabel">Step 2</span>
                <strong>Shape access slots</strong>
          ...
          <section className="builderSectionCard" id="assignment-overrides">
            <div className="builderPanelHeader">
              <div>
                <span className="builderFlowLabel">Step 3</span>
                <strong>Fine-tune slot routing</strong>
            ...
            {detail.assignment_mode === "selected_students" && activeAssignedStudents.length ? (
              <>
                ...
                <div className="builderAssignmentActionsGrid">
          ...
          <section className="builderSectionCard" id="assignment-audit">
            <div className="builderPanelHeader">
              <div>
                <span className="builderFlowLabel">Step 4</span>
                <strong>Review recent slot operations</strong>
          ...
          <section className="builderSectionCard" id="assignment-support">
            <div className="builderPanelHeader">
              <div>
                <span className="builderFlowLabel">Step 5</span>
        ...
        <article className="dashboardPanel builderPanel">
          <div className="builderPanelHeader">
            <div>
              ...
              <p>Review the institute-scoped question inventory tied to this exam’s program and subject before attaching items.</p>
            </div>
            <div className="builderPanelMetrics">
              <article className="builderMetricChip">
    ...
    <div className="studentPage studentDashboardModern instituteConsolePage instituteExamBuilderPageVivid">
      ...
      <section className="studentInsightHeroCard studentInsightHeroCardCompact">
        ...
        <div className="studentInsightHeroActions">
          <Link className="button buttonPrimary" href={`/institute/exams/${detail.id}`}>
            Open Delivery View
          </Link>
      ...
      <section className="teacherResultsReadinessBoard">
        <article className="teacherResultsReadinessHero">
          <span className="studentDashboardTag">Builder readiness</span>
          <strong>
            {resultSummary?.results_published
              ? "Published"
          ...
          <p>See which setup, review, or publication conditions still need attention before moving this exam forward.</p>
        </article>
        <article className="teacherResultsReadinessCard teacherResultsReadinessCardBlocked">
          ...
          {readinessSnapshot.blockers.length ? (
            ...
            <p>No hard blocker is visible on this builder right now.</p>
          )}
        </article>
        ...
        <article className="teacherResultsReadinessCard">
          ...
          {readinessSnapshot.pending.length ? (
            ...
            <p>No extra pending step stands out here.</p>
          )}
        </article>
        ...
        <section className="builderActionCallout">
          <div>
            ...
            <p>
              ...
              attach items from the institute question bank.
            </p>
          </div>
          ...
          <section className="contentCard">
            <div className="builderHeroCard">
              ...
              <div className="resultCardActions">
                ...
                <Link className="button buttonGhost" href={`/institute/exams/${detail.id}`}>
                  Open delivery view
                </Link>
              </div>
            </div>
