...
export async function PATCH(
  request: NextRequest,
  context: { params: Promise<{ starPackId: string }> },
) {
  if (!API_BASE_URL) {
  ...
  const { starPackId } = await context.params;
  const body = await request.json().catch(() => ({}));
  const response = await fetch(`${API_BASE_URL}/api/v1/economy/admin/star-packs/${starPackId}/`, {
    method: "PATCH",
    headers: {
