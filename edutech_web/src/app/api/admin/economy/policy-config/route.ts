...
async function forwardRequest(request: NextRequest, method: "GET" | "PATCH") {
  if (!API_BASE_URL) {
  ...
  const body = method === "PATCH" && request.body ? await request.json().catch(() => ({})) : undefined;
  const response = await fetch(`${API_BASE_URL}/api/v1/economy/admin/policy-config/`, {
    method,
    headers: {
