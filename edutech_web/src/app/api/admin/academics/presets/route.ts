...
async function proxyPresetRequest(request: NextRequest, method: "GET" | "POST") {
  if (!API_BASE_URL) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/v1/academics/presets/${query ? `?${query}` : ""}`, {
    method,
    headers: {
...
export async function GET(request: NextRequest) {
  return proxyPresetRequest(request, "GET");
}
