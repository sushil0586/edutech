import { NextResponse } from "next/server";
import { getAuthenticatedSession, hasRequiredRole, PORTAL_ROLE_GROUPS } from "@/lib/auth/session";

const API_BASE_URL = (
  process.env.API_BASE_URL ?? process.env.NEXT_PUBLIC_API_BASE_URL ?? ""
).replace(/\/$/, "");

async function proxyQuestionRequest(
  method: "GET" | "DELETE",
  questionId: string,
) {
  if (!API_BASE_URL) {
    return NextResponse.json(
      { detail: "Portal API is not configured." },
      { status: 500 },
    );
  }

  const session = await getAuthenticatedSession();
  if (!session || !hasRequiredRole(session.profile, PORTAL_ROLE_GROUPS.teacherScopedOperators)) {
    return NextResponse.json(
      { detail: "Portal session is not available." },
      { status: 401 },
    );
  }

  const response = await fetch(`${API_BASE_URL}/api/v1/question-bank/questions/${questionId}/`, {
    method,
    headers: {
      Authorization: `Bearer ${session.accessToken}`,
      "Content-Type": "application/json",
    },
    cache: "no-store",
  });

  const text = await response.text();
  return new NextResponse(text, {
    status: response.status,
    headers: {
      "Content-Type": response.headers.get("content-type") ?? "application/json",
    },
  });
}

export async function GET(
  _request: Request,
  {
    params,
  }: {
    params: Promise<{ questionId: string }>;
  },
) {
  const { questionId } = await params;
  return proxyQuestionRequest("GET", questionId);
}

export async function DELETE(
  _request: Request,
  {
    params,
  }: {
    params: Promise<{ questionId: string }>;
  },
) {
  const { questionId } = await params;
  return proxyQuestionRequest("DELETE", questionId);
}
