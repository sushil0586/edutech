...
async function createExamAction(formData: FormData) {
  ...
  if (!profile.institute) {
    redirect("/teacher/exams?error=Teacher%20institute%20scope%20is%20missing.");
  }
  ...
  try {
    ...
    redirect(`/teacher/exams/new?error=${encodeURIComponent(message)}`);
  }
}
    ...
    <div className="studentPage studentPageTight createExamPage teacherConsolePage teacherExamBuilderPageVivid">
      ...
      <section className="builderSummaryGrid">
        <article className="builderSummaryCard">
          ...
          <small>Academic year options currently visible in teacher scope</small>
        </article>
        <article className="builderSummaryCard">
          <span>Programs</span>
      ...
      <section className="builderFlowContent builderFlowContentSolo">
          ...
          sourceHelpText="Teacher source keeps the exam teacher-owned in learner views. Institute source publishes it as institute-owned content."
          sourceOptions={[
