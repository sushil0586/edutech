...
type TeacherResultsExamCard = Awaited<ReturnType<typeof fetchTeacherExams>>[number];
type TeacherAttempt = Awaited<ReturnType<typeof fetchTeacherExamAttemptPage>>["results"][number];
type TeacherResultExamFilter = "all" | "published" | "ready" | "live" | "draft";
type TeacherResultExamSort = "latest" | "attempts" | "average" | "title";
type TeacherResultExamGroup = "none" | "publication" | "status";
type TeacherAttemptReviewFilter =
  | "all"
  | "low_performers"
  | "skipped_heavy"
  | "critical"
  ...
  | "in_progress"
  | "auto_submitted";
type TeacherAttemptSort = "latest" | "score_low" | "warnings_high" | "time_long";
type TeacherAttemptGroup = "none" | "health" | "status";
...
function resolveTeacherResultExamFilter(value?: string): TeacherResultExamFilter {
  switch (value) {
    case "published":
    case "ready":
    case "live":
...
function resolveTeacherResultExamSort(value?: string): TeacherResultExamSort {
  switch (value) {
    case "attempts":
    case "average":
    case "title":
...
function resolveTeacherResultExamGroup(value?: string): TeacherResultExamGroup {
  switch (value) {
    case "publication":
    case "status":
      return value;
...
function resolveTeacherAttemptReviewFilter(value?: string): TeacherAttemptReviewFilter {
  switch (value) {
    case "low_performers":
    case "skipped_heavy":
    case "critical":
...
function resolveTeacherAttemptSort(value?: string): TeacherAttemptSort {
  switch (value) {
    case "score_low":
    case "warnings_high":
    case "time_long":
...
function resolveTeacherAttemptGroup(value?: string): TeacherAttemptGroup {
  switch (value) {
    case "health":
    case "status":
      return value;
...
function buildTeacherResultsHref(args: {
  examId?: string;
  attemptId?: string | null;
  attemptFilter?: TeacherAttemptReviewFilter;
  attemptSort?: TeacherAttemptSort;
  attemptGroup?: TeacherAttemptGroup;
  attemptPage?: number;
  attemptPageSize?: number;
  questionFilter?: string;
  examListFilter?: TeacherResultExamFilter;
  examListSort?: TeacherResultExamSort;
  examListGroup?: TeacherResultExamGroup;
  examPage?: number;
  examPageSize?: number;
  ...
  return buildFilterHref("/teacher/results", [
    ["exam", args.examId],
...
function filterTeacherResultExamCards(
  cards: Array<{ exam: TeacherResultsExamCard; summary: TeacherResultSummary | null }>,
  filter: TeacherResultExamFilter,
) {
...
function sortTeacherResultExamCards(
  cards: Array<{ exam: TeacherResultsExamCard; summary: TeacherResultSummary | null }>,
  sortBy: TeacherResultExamSort,
) {
...
function buildTeacherResultExamGroupLabel(
  card: { exam: TeacherResultsExamCard; summary: TeacherResultSummary | null },
  groupBy: TeacherResultExamGroup,
) {
  if (groupBy === "publication") {
    return examPublicationState(card.summary).label;
  }
  if (groupBy === "status") {
    return card.exam.status.replaceAll("_", " ");
  }
  return "Exams";
}

function groupTeacherResultExamCards(
  cards: Array<{ exam: TeacherResultsExamCard; summary: TeacherResultSummary | null }>,
  groupBy: TeacherResultExamGroup,
) {
  if (groupBy === "none") {
    return [{ label: "All exams", items: cards }];
  }

  const buckets = new Map<string, Array<{ exam: TeacherResultsExamCard; summary: TeacherResultSummary | null }>>();
  for (const card of cards) {
    const label = buildTeacherResultExamGroupLabel(card, groupBy);
    buckets.set(label, [...(buckets.get(label) ?? []), card]);
  }
  return Array.from(buckets.entries()).map(([label, items]) => ({ label, items }));
}

function filterTeacherAttempts(attempts: TeacherAttempt[], filter: TeacherAttemptReviewFilter) {
...
function sortTeacherAttempts(attempts: TeacherAttempt[], sortBy: TeacherAttemptSort) {
...
function buildTeacherAttemptGroupLabel(attempt: TeacherAttempt, groupBy: TeacherAttemptGroup) {
  if (groupBy === "health") return healthLabel(attemptHealth(attempt));
  if (groupBy === "status") return attempt.status.replaceAll("_", " ");
  return "Attempts";
}

function groupTeacherAttempts(attempts: TeacherAttempt[], groupBy: TeacherAttemptGroup) {
  if (groupBy === "none") {
    return [{ label: "All attempts", items: attempts }];
  }
  const buckets = new Map<string, TeacherAttempt[]>();
  for (const attempt of attempts) {
    const label = buildTeacherAttemptGroupLabel(attempt, groupBy);
...
export default async function TeacherResultsPage({
  ...
  const attemptFilter = resolveTeacherAttemptReviewFilter(
    readSingle(resolvedSearchParams.attempt_filter) || "all",
  );
  const attemptSort = resolveTeacherAttemptSort(
    readSingle(resolvedSearchParams.attempt_sort) || "latest",
  );
  const attemptGroup = resolveTeacherAttemptGroup(
  ...
  const examListFilter = resolveTeacherResultExamFilter(
    readSingle(resolvedSearchParams.exam_list_filter) || "all",
  );
  const examListSort = resolveTeacherResultExamSort(
    readSingle(resolvedSearchParams.exam_list_sort) || "latest",
  );
  const examListGroup = resolveTeacherResultExamGroup(
  ...
  const visibleExamCards = sortTeacherResultExamCards(
    filterTeacherResultExamCards(resultExamCards, examListFilter),
    examListSort,
  );
  ...
  const groupedExamCards = groupTeacherResultExamCards(pagedExamCards, examListGroup);
  ...
  const groupedAttempts = groupTeacherAttempts(attempts, attemptGroup);
    ...
    <div className="studentPage studentPageTight studentDashboardModern">
      ...
      <section className="studentInsightHeroCard studentInsightHeroCardCompact">
        <div className="studentInsightHeroCopy">
          ...
          <strong>Teacher result operations</strong>
          <small>
            ...
            <form className="workspaceFiltersForm" method="GET">
              ...
              <label className="workspaceFilterField">
                ...
                <select defaultValue={attemptGroup} name="attempt_group">
                  <option value="none">No grouping</option>
                  <option value="health">Teacher health</option>
